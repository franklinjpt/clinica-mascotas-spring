...
function traerDatos(textStatus){
        var pathname = window.location.pathname;
        if(pathname == "/listarPropietario" || pathname == "/agregarMascota" || pathname == "/index"){
...
function mascotaEditada(){
    $('#m-tabla').on('click', '.guardar-m', function(e){
        ...
        if(($(".edi-mas-nombre").val().length < 1) || ($(".edi-mas-fecha").val().length < 1) ) {
                    swal("Error!", "Todos los campos son obligatorios!", "error");
                    return false;
        }
